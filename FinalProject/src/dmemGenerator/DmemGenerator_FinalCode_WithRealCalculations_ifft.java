...
public class DmemGenerator_FinalCode_WithRealCalculations_ifft {
	...
	public static void main(String[] args) throws ParseException, IOException {
		DmemGenerator_FinalCode_WithRealCalculations_ifft obj=new DmemGenerator_FinalCode_WithRealCalculations_ifft();
...
//			else {stage0flag=false;}
			...
			for(int p=0; p<end;p++){
				...
				for(int q=0;q<end2;q++){		
					...
					printCaseStatements(writer,input1,input2,tstage,q,(tstage+1==numberOfStages && p==0 && q==0),(tstage==0 && p==0 && q==0) );	
	...
	private void printCaseStatements(BufferedWriter writer, int input1, int input2, int currentStage, int q, boolean lastStage,boolean firstStage) throws IOException {
		...
		case 512:
			...
			imag=Twiddles_512_ifft.generate(true,currentStage,q);
			break;
		default:
			break;
		}
...
//	}	
	...
	private void printHardCodeStatements(BufferedWriter writer) throws IOException {
		...
		writer.write("reg["+(numberOfBitsToRepresentNPoint-1)+":0] inputAddressCounter;"
		...
		for(int i=0;i<noOfMultipliers;i++){
			writer.write("\n\t\t\t\t\tif(pushoutB"+i+") begin");
			writer.write("\n\t\t\t\t\t\trealOutputBuff[reverseAddress(stateOp1B"+i+")]<=#1 ReOp1B"+i+";");
			writer.write("\n\t\t\t\t\t\trealOutputBuff[reverseAddress(stateOp2B"+i+")]<=#1 ReOp2B"+i+";");
			writer.write("\n\t\t\t\t\t\timagOuptutBuff[reverseAddress(stateOp1B"+i+")]<=#1 imgOp1B"+i+";");
			writer.write("\n\t\t\t\t\t\timagOuptutBuff[reverseAddress(stateOp2B"+i+")]<=#1 imgOp2B"+i+";");
			writer.write("\n\t\t\t\t\tend\n");
		}
